...
## KURAL-025: Her Görev Sonunda Görsel Doğrulama Zorunlu
- Kural: "Tamamlandı" veya "PASS" yazmak yeterli değildir. Her UI değişikliği içeren görev sonunda Playwright görsel testleri çalıştırılır, ekran görüntüleri alınır ve masaüstüne kaydedilir.
- Sebep: Codex kendi yazdığı testleri kendisi geçirebilir. Gerçek doğrulama tarayıcıda gözle görülür çıktıyla yapılır.
- Kontrol: `screenshots/` klasöründe o göreve ait en az 5 ekran görüntüsü olmalı. Yoksa görev tamamlanmış sayılmaz.
- Eklendi: 14.06.2026 — görsel doğrulama görevi

## KURAL-026: Playwright Test Paketi Her Zaman Güncel
- Kural: Yeni UI bileşeni veya ekran eklenince `tests/visual-rules.spec.js` güncellenir. Yeni ekran için ekran görüntüsü testi eklenir.
- Sebep: Test edilmeyen ekran kör noktadır.
- Kontrol: Her yeni `renderXxx()` fonksiyonu için en az 1 Playwright testi olmalı.
- Eklendi: 14.06.2026

## KURAL-027: Görsel Test Çıktıları Masaüstüne Kaydedilir
- Kural: Her Playwright test çalıştırmasında tüm ekran görüntüleri `C:\Users\İRFAN AYYILDIZ\Desktop\kasam-test\visual-test-[YYYY-MM-DD-HHMM]\` klasörüne kopyalanır.
- Sebep: Proje sahibi görselleri buraya yükleyerek gerçek doğrulama yapar.
- Kontrol: Her "BLOK X TAMAMLANDI" mesajında "Görseller: [klasör yolu]" satırı olmalı.
- Eklendi: 14.06.2026

## KURAL-028: Test Türleri Ayrı Raporlanır
- Kural: Her görev sonunda test raporu üç başlıkla ayrılır: `LOCAL SİMÜLASYON`, `GÖRSEL DOĞRULAMA`, `CLOUD TEST`. "Tamamlandı" mesajı bu 3 başlığı içermeden geçerli sayılmaz.
- Sebep: "PASS" yazısı gerçek çalışmayı garantilemez.
- Kontrol: Görev sonu raporunda üç test başlığı açıkça yer alır.
- Eklendi: 14.06.2026

## KURAL-044: Canlı Çok Kullanıcılı İşlev Simülasyonu
...
## KURAL-046: Gerçek Cloud Çok Kullanıcılı Test Zorunlu
...
## KURAL-048: Cloud Live Smoke Test Secretsiz PASS Üretemez
- Kural: Gerçek Supabase çok kullanıcılı doğrulama `scripts/cloud-live-smoke.cjs` ile yapılır. Script ya iki ayrı gerçek test hesabını ortam değişkenlerinden alır ya da local-only `KASAM_SUPABASE_SERVICE_ROLE_KEY` ile geçici iki test kullanıcısı oluşturur. Her iki modda da gerçek Auth oturumu açar, ortak proje/hareket/bildirim yazar, ikinci kullanıcıdan okur ve test verisini temizler. Gerekli secret/env yoksa test `PASS` üretemez.
- Sebep: Simülatör ve localStorage akışı gerçek telefonlar arası cloud davranışını kanıtlamaz. Aynı zamanda test şifreleri ve service role key koda, repo geçmişine, frontend'e veya Vercel client ortamına yazılmamalıdır.
- Kontrol: `npm run test:cloud-live` yalnızca `KASAM_CLOUD_EMAIL_A`, `KASAM_CLOUD_PASSWORD_A`, `KASAM_CLOUD_EMAIL_B`, `KASAM_CLOUD_PASSWORD_B` verildiğinde veya local `KASAM_SUPABASE_SERVICE_ROLE_KEY` bulunduğunda çalışır. Eksik env durumunda exit code 2 ile durur. Windows kullanıcı akışı için `npm run test:cloud-live:prompt` güvenli prompt ile aynı testi çalıştırır.
- Eklendi: 2026-06-21 — Gerçek cloud doğrulama kapısı
...
## KURAL-039: Ekran Kalabaligi Yasak
- Kural: Bir ekranda ayni bilgiyi tekrar isteyen, kullanicinin zaten girdigi veriyi yeniden soran, ana isi bolen veya ikincil ayarlari surekli gorunur tutan alanlar kullanilmaz. Gelir/gider, tutar, tarih, baslik, paylasim, medya ve bildirim oyunu gibi detaylar adim adim ya da acilir/kompakt alanlarda verilir. Bir ozellik sadece ilgili anda gorunur; varsayilan ekran sade kalir.
