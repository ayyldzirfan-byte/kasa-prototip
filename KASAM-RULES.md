...
## KURAL-048: Cloud Live Smoke Test Şifresiz Çalışmaz
- Kural: Gerçek Supabase çok kullanıcılı doğrulama `scripts/cloud-live-smoke.cjs` ile yapılır. Script iki ayrı test hesabını ortam değişkenlerinden alır, gerçek Auth oturumu açar, ortak proje/hareket/bildirim yazar, ikinci kullanıcıdan okur ve test projesini temizler. Gerekli env yoksa test `PASS` üretemez.
- Sebep: Simülatör ve localStorage akışı gerçek telefonlar arası cloud davranışını kanıtlamaz. Aynı zamanda test şifreleri koda veya repo geçmişine yazılmamalıdır.
- Kontrol: `npm run test:cloud-live` yalnızca `KASAM_CLOUD_EMAIL_A`, `KASAM_CLOUD_PASSWORD_A`, `KASAM_CLOUD_EMAIL_B`, `KASAM_CLOUD_PASSWORD_B` verildiğinde çalışır; eksik env durumunda exit code 2 ile durur. Windows kullanıcı akışı için `npm run test:cloud-live:prompt` güvenli prompt ile aynı testi çalıştırır.
