...
const result = JSON.parse(JSON.stringify(vm.runInContext(`
  ...
  function periodSummary(period, userId) {
    ...
    return {
      ...
      htmlHasIncome: html.includes("Gelir"),
      htmlHasExpense: html.includes("Gider"),
...
const lines = [
  ...
  "",
  `- Gelir: ${result.partnerDay.income.toLocaleString("tr-TR")} TL`,
  `- Gider: ${result.partnerDay.expense.toLocaleString("tr-TR")} TL`,
